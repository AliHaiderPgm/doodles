...
const Card = ({ icon: Icon, iconColor = 'text-blue-500', title, desc, image, component, className, heading, comment, getTemplate = false, hoverable = false }) => {
    return (
        <div className={`bg-gray-800/5 dark:bg-gray-800/80 rounded-xl shadow
        ${className} 
        ...
        ${image ? "grid grid-rows-3 lg:grid-rows-4 grid-cols-6" : ""}`
            ...
            {image &&
                <div className='row-span-2 lg:row-span-3 col-span-5 col-start-2 relative'>
                    <img src={image} alt={title} className='h-full w-full object-cover object-left-top ms-auto absolute bottom-0 right-0 rounded-tl-md' height={700} width={500} />
                </div>
            }
            {/* Carousel */}
            {component && <div className='pb-4 px-2 lg:px-6 lg:p-6'>{component}</div>}
            {/* Comments */}
            {comment && <div className='p-6 space-y-2'>
                <div className='flex gap-2'>
                    <img src={comment.profileImage} alt={comment.author} className='h-12 w-12 rounded-full overflow-hidden border bg-white p-1' height={30} width={30} />
                    <div>
